                  ...
                  Bool_t silent)
{
   ...
   if (fState!=kForwardDeclared && !fClassInfo) {
      ...
      if (fState == kHasTClassInit) {
         ...
         // Here we check and grab the info from the pcm.
...
ptrdiff_t TClass::GetBaseClassOffsetRecurse(const TClass *cl)
{
   ...
   while (lnk) {
      ...
      //"Warning in <TClass::TClass>: no dictionary for class XYZ is available"
...
TList *TClass::GetListOfEnums(Bool_t requestListLoading /* = kTRUE */)
{
   ...
   if (!requestListLoading) {
      ...
      if (fromRootCling) // rootcling is single thread
...
Bool_t TClass::IsFolder(void *) const
{
   return false;
}
...
/// marked for I/O. In other words, if this routine returns true, TClass::New is
